                          ...
                          <table class="table-auto w-full text-left whitespace-no-wrap">
                            ...
                            <tbody>
                              <tr v-for="order in props.orders.data" :key="order.id">
                                <td class="border-t-2 border-gray-200px-4 py-3">
                                  <Link class="text-blue-400" :href="route('purchases.show', { purchase: order.id })">
                                    {{ order.id }}
                                  </Link>
                                </td>
                                <td class="border-t-2 border-gray-200px-4 py-3">{{ order.customer_name }}</td>
                                <td class="border-t-2 border-gray-200px-4 py-3">{{ order.total }}</td>
                                <td class="border-t-2 border-gray-200px-4 py-3">{{ order.status }}</td>
                                <td class="border-t-2 border-gray-200px-4 py-3">{{ dayjs(order.created_at).format('YYYY-MM-DD HH:mm:ss') }}</td>
                              </tr>
                            </tbody>
                          </table>
                        </div>
